...
// NewSR201OpenClose initializes a new SR201 instance
func NewSR201OpenClose(ip string, port string) (*SR201, error) {
...
func main() {
	...
	sr201, err := NewSR201OpenClose(SR201_IP, SR201_PORT)
	if err != nil {
	...
	// Close relay 1
	if err := sr201.CloseRelay(1); err != nil {
		fmt.Println("Error closing relay 1:", err)
	} else {
		fmt.Println("Relay 1 closed.")
	}

	// Check status
	statusto, err := sr201.CheckStatus()
	if err != nil {
		fmt.Println("Error checking status:", err)
	} else {
		fmt.Printf("Relay status: %s\n", statusto)
	...
	// Open relay 1
	time.Sleep(5 * time.Second) // Simulate a delay before opening
	if err := sr201.OpenRelay(1); err != nil {
		fmt.Println("Error opening relay 1:", err)
	} else {
		fmt.Println("Relay 1 opened.")
	}

	// Check status
	statustre, err := sr201.CheckStatus()
	if err != nil {
		fmt.Println("Error checking status:", err)
	} else {
		fmt.Printf("Relay status: %s\n", statustre)
